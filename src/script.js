...
window.onload = function load() {
  ...
  for (let box = 0; box < boxes.length; box += 1) {
    ...
    boxes[box].addEventListener('keydown', ((ev, boxes, box) => {
      // if enter is pressed
      if (ev.which === 13) {
        addPlayingPiece(boxes[box], convertToCoords(box));
      }
    }).bind(undefined, ev, boxes, box));
  }
};
...
/**
 * Determine whose go it is, add an appropriate image to the box that was clicked on
...
/**
 * Check for line of 3 consecutive playing pieces, if winner found then
 *  alert the user, then reset the game
...
function checkForWinner(boxClicked, player) {
  ...
  directions.forEach((dir) => {
    const ansDir = searchInDirection(boxClicked, dir, player);
    if (ansDir) {
      answers.push(ansDir);
    }
  });
...
function searchInDirection(start, direction, player) {
  ...
  (function innerL(prev) {
    // advance to next node
    const current = [prev[0] + direction[0][0], prev[1] + direction[0][1]];
    // check one direction
    ...
    if (player === gameboard?.[current[0]]?.[current[1]]) {
      ...
      return innerL(current);
    }
  }(start));

  (function innerR(prev) {
    const current = [prev[0] + direction[1][0], prev[1] + direction[1][1]];
    if (player === gameboard?.[current[0]]?.[current[1]]) {
      // prepend to hits list
      history.unshift(current);
      return innerR(current);
    }
  }(start));
...
/* eslint-disable consistent-return */
/**
 * Convert a number 0-9 to coordinates on a 3x3 grid, with top left being [0,0]
 *  and bottom right [2,2]
